...
const ProductDetail = () => {
  ...
  const { productIds, add: addCompare, remove: removeCompare } = useCompare();
    ...
    <div className="container-custom px-4 md:px-8 py-8">
      ...
      <div className="grid md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-5">
          ...
          <div className="flex items-center gap-2 mt-6">{v.stock > 0 ? <><QuantityCounter value={qty} onChange={setQty} max={v.stock} /><button onClick={() => { addItem(product, qty); setQty(1); }} className="btn-primary flex-1 !py-3.5">Add to Cart — {formatPrice(price * qty)}</button><button onClick={toggleWish} disabled={wishToggling} className={`btn-icon !w-12 !h-12 flex-shrink-0 ${wished ? 'text-error bg-error/5' : 'text-text-muted'}`} aria-label={wished ? 'Remove from wishlist' : 'Add to wishlist'}><svg className="w-5 h-5" fill={wished ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg></button><button onClick={() => productIds.includes(id) ? removeCompare(id) : addCompare(id)} className={`btn-icon !w-12 !h-12 flex-shrink-0 ${productIds.includes(id) ? 'bg-primary text-white' : 'text-text-muted'}`} aria-label="Compare"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg></button></> : <button disabled className="btn-primary flex-1 !py-3.5 opacity-50 cursor-not-allowed">Out of Stock</button>}</div>
