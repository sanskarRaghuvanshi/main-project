      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <div className="flex items-center gap-1">
            <button onClick={e => { e.preventDefault(); productIds.includes(product._id) ? removeCompare(product._id) : addCompare(product._id); }}
              className={`text-xs px-2 py-1 rounded-lg border transition ${productIds.includes(product._id) ? 'bg-primary text-white border-primary' : 'border-border text-text-muted hover:border-primary hover:text-primary'}`}
              aria-label="Compare">{productIds.includes(product._id) ? '✓' : '⇆'}</button>
